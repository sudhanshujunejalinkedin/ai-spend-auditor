        ...
        <div className="px-6 py-6 space-y-6">
          <div className="bg-zinc-900 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:justify-between">
            ...
            <div className="flex gap-4 sm:gap-6 text-center">
              <div><p className="text-white font-bold text-xl">5</p><p className="text-zinc-400 text-xs">Tools Found</p></div>
              <div><p className="text-amber-400 font-bold text-xl">3</p><p className="text-zinc-400 text-xs">Actions Needed</p></div>
              <div><p className="text-green-400 font-bold text-xl">1</p><p className="text-zinc-400 text-xs">Keep As-Is</p></div>
            </div>
          </div>
                ...
                return (
                  <div key={tool.name} className="border border-zinc-100 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3 hover:border-zinc-200 transition-colors">
                    <div className="flex-1 min-w-0">
                      ...
                      {tool.alt && <p className="text-zinc-400 text-xs mt-0.5">→ Switch to: <span className="text-zinc-600 font-medium">{tool.alt}</span></p>}
                    </div>
                    <div className="flex items-center gap-4 shrink-0">
                      <div className="text-center"><p className="text-xs text-zinc-400">Cost/seat</p><p className="font-bold text-zinc-900 text-sm">${tool.cost}/mo</p></div>
                      <div className="text-center"><p className="text-xs text-zinc-400">Seats</p><p className="font-bold text-zinc-900 text-sm">{tool.seats}</p></div>
                      <div className="text-center">
                        <p className="text-xs text-zinc-400">Usage</p>
                        <div className="flex items-center gap-1">
                          <div className="w-14 h-1.5 rounded-full bg-zinc-100">
                            <div className={`h-1.5 rounded-full ${tool.usage > 50 ? "bg-green-500" : tool.usage > 20 ? "bg-amber-400" : "bg-red-400"}`} style={{ width: `${tool.usage}%` }} />
                          </div>
...
export default function LandingPage() {
  ...
  return (
    <div className="relative min-h-screen bg-white text-zinc-900 flex flex-col items-center overflow-hidden font-sans">
      ...
      {/* ───── HEADER ───── */}
      <header className="w-full sticky top-0 z-40 bg-white/80 backdrop-blur-sm border-b border-zinc-100">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <nav className="flex items-center gap-8 text-xs font-semibold uppercase tracking-widest text-zinc-500">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-5 py-2.5 rounded-full border border-zinc-200 text-zinc-600 font-semibold text-xs uppercase tracking-widest hover:border-zinc-400 hover:text-zinc-900 transition-all"
              >
                See Example Report
              </button>
              <Link href="/audit">
                <button className="px-5 py-2.5 rounded-full bg-zinc-900 text-white font-semibold text-xs uppercase tracking-widest hover:bg-green-600 transition-all">
                  Start Free Audit →
                </button>
              </Link>
            </div>
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white border-b border-zinc-100 px-6 py-5 space-y-4">
            <a href="#how-it-works" onClick={() => setIsMobileMenuOpen(false)} className="block text-xs font-semibold uppercase tracking-widest text-zinc-500 hover:text-zinc-900 transition-colors">How It Works</a>
            <a href="#features" onClick={() => setIsMobileMenuOpen(false)} className="block text-xs font-semibold uppercase tracking-widest text-zinc-500 hover:text-zinc-900 transition-colors">Features</a>
            <button
              onClick={() => { setShowReport(true); setIsMobileMenuOpen(false); }}
              className="w-full border border-zinc-200 text-zinc-700 py-3 rounded-2xl font-bold text-sm hover:border-zinc-400 transition-all"
            >
              See Example Report
            </button>
            <Link href="/audit" className="block">
              <button className="w-full bg-zinc-900 text-white py-3 rounded-2xl font-bold text-sm hover:bg-green-600 transition-all">
                Start Free Audit →
              </button>
            </Link>
          </div>
      ...
      {/* ───── FEATURES ───── */}
      <section id="features" className="w-full bg-zinc-950 py-16 sm:py-24">
